...
interface TokenInterface {
 ...
 contract DayDayCrowdsale is Ownable{
  ...
  function DayDayCrowdsale(uint256 _startTime, address _wallet, address _tokenAddress) public 
  {
    ...
    maxTokensToSaleInPrivateSeedPhase1 = 199500000 * 10 ** 18;
    maxTokensToSaleInPrivateSeedPhase2 = 388500000 * 10 ** 18;
    maxTokensToSaleInSyndicagteSeed = 924000000 * 10 ** 18; 
    maxTokensToSaleInPreITOPublic = 1396500000 * 10 ** 18;
    maxTokensToSaleInITOPublicPhase1 = 1790250000 * 10 ** 18;
    maxTokensToSaleInITOPublicPhase2 = 2294250000 * 10 ** 18;
    maxTokensToSaleInITOPublicPhase3 = 2898000000 * 10 ** 18;
    maxTokensToSale = 3000000000 * 10 ** 18;
   ...
   function () public  payable {
    ...
    function determineBonus(uint tokens) internal view returns (uint256 bonus) 
    {
        ...
        //Syndicagte phase (15 days)
        else if (timeElapsedInDays >= 16 && timeElapsedInDays<32)
        {
            //bonus application of SyndicagteSeed
        ...
        else if (timeElapsedInDays >= 32 && timeElapsedInDays<103)
        {
            //71 days break
  ...
  function hasEnded() public constant returns (bool) {
    ...
    **/
    ...
    function changeHardCap(uint256 hardCapEthers) public onlyOwner
    {
     ...
     {
         token.transfer(F1,90000000 * 10 ** 18);
         token.transfer(F2,60000000 * 10 ** 18);
     ...
     {
         token.transfer(A1,90000000 * 10 ** 18);
         token.transfer(A2,60000000 * 10 ** 18);
         token.transfer(A3,30000000 * 10 ** 18);
         token.transfer(A4,60000000 * 10 ** 18);
         token.transfer(A5,60000000 * 10 ** 18);
         token.transfer(A6,15000000 * 10 ** 18);
         token.transfer(A7,15000000 * 10 ** 18);
         token.transfer(A8,15000000 * 10 ** 18);
         token.transfer(A9,15000000 * 10 ** 18);
     ...
     {
         token.transfer(C1,2500000 * 10 ** 18);
         token.transfer(C2,1000000 * 10 ** 18);
         token.transfer(C3,1500000 * 10 ** 18);
         token.lockTokensForCs(C1,C2,C3);
     }
      function tokensAllocatedForTeamAndReserve(address team) public onlyOwner
     {
         token.transfer(team,63000000 * 10 ** 18);
